...
{
    Actuator_Context_work_param_size_check(src.length);
    int i_1;
...
{
    Controller_Context_work_param_size_check(src.length);
    int i_1;
    for(i_1 : 0 .. (src.length - 1))
    {
        dst.data[i_1] = src.data[i_1];
...
{
    MyTestParam_size_check(src.length);
    int i_0;
...
inline MyTestParam_size_check(MyTestParam_sc)
{
    assert(((MyTestParam_sc >= 1) && (MyTestParam_sc <= 5)));
}
