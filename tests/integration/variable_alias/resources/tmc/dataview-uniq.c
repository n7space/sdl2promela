...
{
    flag ret = TRUE;
    if (pVal->kind == MyUnion_left_PRESENT) {
    	ret = asn1SccMyInteger_IsConstraintValid((&(pVal->u.left)), pErrCode);
    }
    if (ret) {
        if (pVal->kind == MyUnion_right_PRESENT) {
        	ret = asn1SccMySeq_IsConstraintValid((&(pVal->u.right)), pErrCode);
        }
    }   /*COVERAGE_IGNORE*/
...
void asn1SccMyUnion_Initialize(asn1SccMyUnion* pVal)
{
	...
	pVal->kind = MyUnion_left_PRESENT;
	pVal->u.left = 0UL;
}
...
flag asn1SccMyUnion_Encode(const asn1SccMyUnion* pVal, BitStream* pBitStrm, int* pErrCode, flag bCheckConstraints)
{
	...
	*pErrCode = 0;
	...
	if (ret && *pErrCode == 0) {
	    switch(pVal->kind)
	    {
	    case MyUnion_left_PRESENT:
	    	BitStream_EncodeConstraintWholeNumber(pBitStrm, 0, 0, 1);
	    	ret = asn1SccMyInteger_Encode((&(pVal->u.left)), pBitStrm, pErrCode, FALSE);
	    	break;
	    case MyUnion_right_PRESENT:
	    	BitStream_EncodeConstraintWholeNumber(pBitStrm, 1, 0, 1);
	    	ret = asn1SccMySeq_Encode((&(pVal->u.right)), pBitStrm, pErrCode, FALSE);
	    	break;
	    default:                            /*COVERAGE_IGNORE*/
	...
	*pErrCode = ret ? 0 : ERR_UPER_DECODE_MYUNION;
	if (ret) {
	    ...
	    case 0:
	    	...
	    	break;
	    case 1:
	    	pVal->kind = MyUnion_right_PRESENT;
	    	ret = asn1SccMySeq_Decode((&(pVal->u.right)), pBitStrm, pErrCode);
	    	break;
	    default:                        /*COVERAGE_IGNORE*/
