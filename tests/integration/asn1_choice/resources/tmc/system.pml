...
inline Controller_0_RI_0_work(controller_work_Controller_work_p1)
{
    ...
    c_code {
        ...
        if(now.controller_work_Controller_work_p1_var.selection == MyTestParam_snare_PRESENT)
        {
            controller_work_Controller_work_p1_c_var.u.snare = now.controller_work_Controller_work_p1_var.data.snare;
            
            controller_work_Controller_work_p1_c_var.kind = MyTestParam_snare_PRESENT;
        }
        else if(now.controller_work_Controller_work_p1_var.selection == MyTestParam_kick_PRESENT)
        {
            controller_work_Controller_work_p1_c_var.u.kick = now.controller_work_Controller_work_p1_var.data.kick;
            
            controller_work_Controller_work_p1_c_var.kind = MyTestParam_kick_PRESENT;
        }
        else if(now.controller_work_Controller_work_p1_var.selection == MyTestParam_ride_PRESENT)
        {
            controller_work_Controller_work_p1_c_var.u.ride = now.controller_work_Controller_work_p1_var.data.ride;
            
            controller_work_Controller_work_p1_c_var.kind = MyTestParam_ride_PRESENT;
        }
    ...
    printf("channel_send Actuator_work_channel: %d, %d, %d, %d\n", controller_work_Controller_work_p1_var.data.snare, controller_work_Controller_work_p1_var.data.kick, controller_work_Controller_work_p1_var.data.ride, controller_work_Controller_work_p1_var.selection);
}
...
Actuator_work_loop:
        ...
        ::  nempty(Actuator_work_channel);
            ...
            c_code {
                ...
                if(Actuator_work_signal_parameter_c_var.kind == MyTestParam_snare_PRESENT)
                {
                    now.Actuator_work_signal_parameter.data.snare = Actuator_work_signal_parameter_c_var.u.snare;
                    
                    now.Actuator_work_signal_parameter.selection = MyTestParam_snare_PRESENT;
                }
                 else if(Actuator_work_signal_parameter_c_var.kind == MyTestParam_kick_PRESENT)
                {
                    now.Actuator_work_signal_parameter.data.kick = Actuator_work_signal_parameter_c_var.u.kick;
                    
                    now.Actuator_work_signal_parameter.selection = MyTestParam_kick_PRESENT;
                }
                 else if(Actuator_work_signal_parameter_c_var.kind == MyTestParam_ride_PRESENT)
                {
                    now.Actuator_work_signal_parameter.data.ride = Actuator_work_signal_parameter_c_var.u.ride;
                    
                    now.Actuator_work_signal_parameter.selection = MyTestParam_ride_PRESENT;
                }
                }
            }
            printf("channel_recv Actuator_work_channel: %d, %d, %d, %d\n", Actuator_work_signal_parameter.data.snare, Actuator_work_signal_parameter.data.kick, Actuator_work_signal_parameter.data.ride, Actuator_work_signal_parameter.selection);
