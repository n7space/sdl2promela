...
{
    flag ret = TRUE;
    if (pVal->kind == MyChoice_first_PRESENT) {
    	ret = asn1SccMyInteger_IsConstraintValid((&(pVal->u.first)), pErrCode);
    }
    if (ret) {
        if (pVal->kind == MyChoice_second_PRESENT) {
...
void asn1SccMyChoice_Initialize(asn1SccMyChoice* pVal)
{
	...
	pVal->kind = MyChoice_first_PRESENT;
	pVal->u.first = 0UL;
}
	...
	*pErrCode = ret ? 0 : ERR_UPER_DECODE_MYCHOICE;
	if (ret) {
	    ...
	    case 0:
	    	...
	    	break;
	    case 1:
	    	pVal->kind = MyChoice_second_PRESENT;
