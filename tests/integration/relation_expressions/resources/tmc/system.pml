#include "dataview.pml"
#include "actuator.pml"
#include "controller.pml"
#include "env_inlines.pml"
#include "message_sizes.pml"
c_decl {
    \#include "dataview-uniq.h"
}
typedef MyBooleanResultMessage {
#if asn1SccMyBooleanResult_REQUIRED_BYTES_FOR_ENCODING == 0
    byte data[1];
#else
    byte data[asn1SccMyBooleanResult_REQUIRED_BYTES_FOR_ENCODING];
#endif
}

typedef RelationTestParamMessage {
#if asn1SccRelationTestParam_REQUIRED_BYTES_FOR_ENCODING == 0
    byte data[1];
#else
    byte data[asn1SccRelationTestParam_REQUIRED_BYTES_FOR_ENCODING];
#endif
}

typedef system_state {
    Actuator_Context actuator;
    Controller_Context controller;
    AggregateTimerData timers;
}

int inited;
chan Actuator_check_channel = [1] of {RelationTestParamMessage};
RelationTestParam Actuator_check_signal_parameter;
RelationTestParamMessage Actuator_check_signal_parameter_message;
bool Actuator_check_channel_used = 0;
MyBooleanResultMessage actuator_result_Actuator_result_p1_message;
MyBooleanResult actuator_result_Actuator_result_p1_var;
chan Controller_result_channel = [1] of {MyBooleanResultMessage};
MyBooleanResult Controller_result_signal_parameter;
MyBooleanResultMessage Controller_result_signal_parameter_message;
bool Controller_result_channel_used = 0;
RelationTestParamMessage controller_check_Controller_check_p1_message;
RelationTestParam controller_check_Controller_check_p1_var;
system_state global_state;
chan Actuator_lock = [1] of {int};
chan Controller_lock = [1] of {int};
inline Actuator_0_PI_0_check_unhandled_input(p1)
{
    printf("unhandled_input actuator check\n");
    skip;
}
inline Actuator_0_RI_0_result(actuator_result_Actuator_result_p1)
{
    MyBooleanResult_assign_value(actuator_result_Actuator_result_p1_var, actuator_result_Actuator_result_p1);
    c_code {
        {
        asn1SccMyBooleanResult actuator_result_Actuator_result_p1_c_var;
        BitStream actuator_result_Actuator_result_p1_stream;
        int actuator_result_Actuator_result_p1_rc;
        
        actuator_result_Actuator_result_p1_c_var = now.actuator_result_Actuator_result_p1_var;
        
        BitStream_Init(&actuator_result_Actuator_result_p1_stream,
            now.actuator_result_Actuator_result_p1_message.data,
            asn1SccMyBooleanResult_REQUIRED_BYTES_FOR_ENCODING);
        asn1SccMyBooleanResult_Encode(&actuator_result_Actuator_result_p1_c_var,
            &actuator_result_Actuator_result_p1_stream,
            &actuator_result_Actuator_result_p1_rc,
            0);
        }
    }
    Controller_result_channel!actuator_result_Actuator_result_p1_message;
    printf("channel_send Controller_result_channel: %d\n", actuator_result_Actuator_result_p1_var);
}
inline Actuator_check_queue()
{
    atomic {
        empty(Actuator_check_channel);
    }
}
inline Actuator_0_RI_0_get_sender(Actuator_sender_arg)
{
    skip;
}
inline Controller_0_PI_0_result_unhandled_input(p1)
{
    printf("unhandled_input controller result\n");
    skip;
}
inline Controller_0_RI_0_check(controller_check_Controller_check_p1)
{
    RelationTestParam_assign_value(controller_check_Controller_check_p1_var, controller_check_Controller_check_p1);
    c_code {
        {
        asn1SccRelationTestParam controller_check_Controller_check_p1_c_var;
        BitStream controller_check_Controller_check_p1_stream;
        int controller_check_Controller_check_p1_rc;
        
        controller_check_Controller_check_p1_c_var.oper = now.controller_check_Controller_check_p1_var.oper;
        controller_check_Controller_check_p1_c_var.left = now.controller_check_Controller_check_p1_var.left;
        controller_check_Controller_check_p1_c_var.right = now.controller_check_Controller_check_p1_var.right;
        
        BitStream_Init(&controller_check_Controller_check_p1_stream,
            now.controller_check_Controller_check_p1_message.data,
            asn1SccRelationTestParam_REQUIRED_BYTES_FOR_ENCODING);
        asn1SccRelationTestParam_Encode(&controller_check_Controller_check_p1_c_var,
            &controller_check_Controller_check_p1_stream,
            &controller_check_Controller_check_p1_rc,
            0);
        }
    }
    Actuator_check_channel!controller_check_Controller_check_p1_message;
    printf("channel_send Actuator_check_channel: %d, %d, %d\n", controller_check_Controller_check_p1_var.oper, controller_check_Controller_check_p1_var.left, controller_check_Controller_check_p1_var.right);
}
inline Controller_check_queue()
{
    atomic {
        empty(Controller_result_channel);
    }
}
inline Controller_0_RI_0_get_sender(Controller_sender_arg)
{
    skip;
}
active proctype Actuator_check() priority 1
{
    inited;
    do
    ::  atomic {
        nempty(Actuator_check_channel);
        Actuator_lock?_;
Actuator_check_loop:
        if
        ::  nempty(Actuator_check_channel);
            Actuator_check_channel?Actuator_check_signal_parameter_message;
            c_code {
                {
                asn1SccRelationTestParam Actuator_check_signal_parameter_c_var;
                BitStream Actuator_check_signal_parameter_stream;
                int Actuator_check_signal_parameter_rc;BitStream_AttachBuffer(&Actuator_check_signal_parameter_stream,
                    now.Actuator_check_signal_parameter_message.data,
                    asn1SccRelationTestParam_REQUIRED_BYTES_FOR_ENCODING);
                asn1SccRelationTestParam_Decode(&Actuator_check_signal_parameter_c_var,
                    &Actuator_check_signal_parameter_stream,
                    &Actuator_check_signal_parameter_rc);
                now.Actuator_check_signal_parameter.oper = Actuator_check_signal_parameter_c_var.oper;
                now.Actuator_check_signal_parameter.left = Actuator_check_signal_parameter_c_var.left;
                now.Actuator_check_signal_parameter.right = Actuator_check_signal_parameter_c_var.right;
                }
            }
            printf("channel_recv Actuator_check_channel: %d, %d, %d\n", Actuator_check_signal_parameter.oper, Actuator_check_signal_parameter.left, Actuator_check_signal_parameter.right);
            Actuator_check_channel_used = 1;
            Actuator_0_PI_0_check(Actuator_check_signal_parameter);
            goto Actuator_check_loop;
        ::  empty(Actuator_check_channel);
            skip;
        fi;
        Actuator_lock!1;
    }
    od;
}
active proctype Controller_result() priority 1
{
    inited;
    do
    ::  atomic {
        nempty(Controller_result_channel);
        Controller_lock?_;
Controller_result_loop:
        if
        ::  nempty(Controller_result_channel);
            Controller_result_channel?Controller_result_signal_parameter_message;
            c_code {
                {
                asn1SccMyBooleanResult Controller_result_signal_parameter_c_var;
                BitStream Controller_result_signal_parameter_stream;
                int Controller_result_signal_parameter_rc;BitStream_AttachBuffer(&Controller_result_signal_parameter_stream,
                    now.Controller_result_signal_parameter_message.data,
                    asn1SccMyBooleanResult_REQUIRED_BYTES_FOR_ENCODING);
                asn1SccMyBooleanResult_Decode(&Controller_result_signal_parameter_c_var,
                    &Controller_result_signal_parameter_stream,
                    &Controller_result_signal_parameter_rc);
                now.Controller_result_signal_parameter = Controller_result_signal_parameter_c_var;
                }
            }
            printf("channel_recv Controller_result_channel: %d\n", Controller_result_signal_parameter);
            Controller_result_channel_used = 1;
            Controller_0_PI_0_result(Controller_result_signal_parameter);
            goto Controller_result_loop;
        ::  empty(Controller_result_channel);
            skip;
        fi;
        Controller_lock!1;
    }
    od;
}
init
{
    atomic {
        global_dataview_init();
        Actuator_0_init();
        Actuator_lock!1;
        Controller_0_init();
        Controller_lock!1;
        inited = 1;
    }
}
#include "scl.pml"
